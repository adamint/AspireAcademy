...
[Trait("Category", "E2E")]
public class LessonStateTests(AppHostPlaywrightFixture fixture) : IClassFixture<AppHostPlaywrightFixture>
{
    [Fact]
    public async Task MarkCompleteResets_WhenNavigatingToNextLesson()
    {
        ...
        try
        {
            ...
            await Assertions.Expect(newBtn).ToBeVisibleAsync(new() { Timeout = 20_000 });
            await Assertions.Expect(newBtn).ToContainTextAsync("Mark Complete", new() { Timeout = 10_000 });
    ...
    public async Task SkipLesson_ShowsSkipIconAndUnlocksNext()
    {
        ...
        try
        {
            ...
            await Assertions.Expect(page.GetByText("⏭️")).ToBeVisibleAsync(new() { Timeout = 10_000 });
            ...
            await Assertions.Expect(backBtn).ToBeVisibleAsync(new() { Timeout = 10_000 });
            ...
            await Assertions.Expect(secondLesson).ToBeVisibleAsync(new() { Timeout = 15_000 });
        }
        finally { await fixture.ClosePageAsync(page); }
    }
