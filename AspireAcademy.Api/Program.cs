...
var academyDbConnectionString = builder.Configuration.GetConnectionString("academydb")
    ?? builder.Configuration["ConnectionStrings:academydb"]
    ?? throw new InvalidOperationException("Missing 'ConnectionStrings:academydb' configuration.");

var academyDbBuilder = new NpgsqlConnectionStringBuilder(academyDbConnectionString);
var isAzurePostgres = !string.IsNullOrWhiteSpace(academyDbBuilder.Host) &&
    academyDbBuilder.Host.Contains(".postgres.database.azure.com", StringComparison.OrdinalIgnoreCase);

if (isAzurePostgres)
{
    // Azure PostgreSQL Flexible Server requires TLS + Entra token auth.
    academyDbBuilder.SslMode = SslMode.Require;

    // Prefer explicit username override, otherwise derive the managed identity
    // principal name generated by Aspire deployments: api_identity-<resourceSuffix>.
    var explicitDbUser = builder.Configuration["ACADEMYDB_USERNAME"];
    if (!string.IsNullOrWhiteSpace(explicitDbUser))
    {
        academyDbBuilder.Username = explicitDbUser;
    }
    else
    {
        var host = academyDbBuilder.Host ?? string.Empty;
        ...
        if (match.Success)
        {
            academyDbBuilder.Username = $"api_identity-{match.Groups[1].Value}";
        }
        else if (string.IsNullOrWhiteSpace(academyDbBuilder.Username))
        {
            academyDbBuilder.Username = "app";
        }
    }
    ...
    var dataSourceBuilder = new NpgsqlDataSourceBuilder(academyDbBuilder.ConnectionString);
    dataSourceBuilder.UsePasswordProvider(
        passwordProvider: _ => throw new NotSupportedException("Synchronous token acquisition is not supported."),
    ...
    builder.Services.AddDbContext<AcademyDbContext>(options => options.UseNpgsql(dataSource));
}
else
{
    // Local/dev container database path (Aspire component integration).
